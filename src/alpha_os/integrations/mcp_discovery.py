...
def _parse_hermes_config() -> list[dict[str, Any]]:
    """Hermes stores MCP in active profile + global config under mcp_servers."""
    try:
        import yaml
    except ImportError:
        return []
    ...
    home = hermes_home()
    paths = [hermes_config_path(), home / "config.yaml", home / "config.yml"]
...
def discover_mcp_servers() -> list[dict[str, Any]]:
    ...
    for path in (
        oc_home / "openclaw.json",
        oc_home / "clawdbot.json",
        oc_home / "moltbot.json",
    ):
